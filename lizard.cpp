...
void Cat::catThread (Cat *aCat)
{
	...
	if (debug)
    {
		cout << "[" << aCat->getId() << "] cat is alive"<< endl;
		cout << flush;
    }
	...
	while(running)
    {
	     ...
	     */
      cout <<"S-M:"<<numCrossingSago2MonkeyGrass;
		if (numCrossingSago2MonkeyGrass + numCrossingMonkeyGrass2Sago > MAX_LIZARD_CROSSING)
		{
		  cout << "\tThe cats are happy - they have toys."<< endl;
		  exit( -1 );
		}
    }
}
...
void Lizard::sleepNow()
{
	...
	if (debug)
    {
      cout << "[" << _id << "] lizard sleeping for " << sleepSeconds << " seconds" << endl;
      cout << flush;
    }
